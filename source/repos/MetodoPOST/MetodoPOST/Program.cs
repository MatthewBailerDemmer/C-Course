...
namespace MetodoPOST
{
    internal class Program
    {
        ...
        public static string dadosFormularioGet =
            ...
            "</body>" +
           "</html>";
        public static string dadosFormularioPost =
             "<!DOCTYPE html>" +
            "<html>" +
                "<head>" +
                    "<title>C#completo</title>" +
                ...
                if (req.HttpMethod == "GET")
                {
                    switch (req.Url.AbsolutePath)
                    {
                        ...
                        case "/login":
                            ...
                            break;
                        case "/loginPost":
                            dados = Encoding.UTF8.GetBytes(dadosFormularioPost);
                            break;
                        case "/acao_get_login":
                ...
                else if(req.HttpMethod == "POST")
                {
                    switch (req.Url.AbsolutePath)
                    {
                        case "/acao_post_login":
                            ...
                            NameValueCollection parametrosValores = HttpUtility.ParseQueryString(dadosPost);
                            if (parametrosValores["nome"] == "Marcos" && parametrosValores["senha"] == "1234")
                            {
                               dados = Encoding.UTF8.GetBytes(paginaLoginSucesso);
                            }
